			...
			lock (_sync)
			{
				...
				foreach (ServiceInstance toBeScheduledInstance in servicesForNextTimeLine)
				{
					...
					var servicesWithSameConfiguration = from scheduled in _scheduledServices
														where
															scheduled.Value.Configuration.GetBaseConfiguration(ServiceConfigurationLevel.Template) == toBeScheduledInstance.Configuration.GetBaseConfiguration(ServiceConfigurationLevel.Template) && //should be id but no id yet
					...
					var servicesWithSameProfile = from scheduled in _scheduledServices
												  where
												  scheduled.Value.Configuration.GetBaseConfiguration(ServiceConfigurationLevel.Profile) == toBeScheduledInstance.Configuration.GetBaseConfiguration(ServiceConfigurationLevel.Profile) &&
												  scheduled.Value.State != ServiceState.Ended
												  orderby
												  scheduled.Value.SchedulingInfo.EstimateTimeStart ascending
												  select
					...
					if (!_executionTimeCach.ContainsKey(toBeScheduledInstance.Configuration.GetBaseConfiguration(ServiceConfigurationLevel.Profile)))
						executionTimeInSeconds = toBeScheduledInstance.Configuration.GetBaseConfiguration(ServiceConfigurationLevel.Profile).GetStatistics(_percentile).AverageExecutionTime;
					else
						executionTimeInSeconds = _executionTimeCach[toBeScheduledInstance.Configuration.GetBaseConfiguration(ServiceConfigurationLevel.Profile)];
					...
					while (!found)
					{
						...
						if (countedPerConfiguration < toBeScheduledInstance.Configuration.GetBaseConfiguration(ServiceConfigurationLevel.Profile).Limits.MaxConcurrentGlobal)
						{
							...
							if (countedPerProfile < toBeScheduledInstance.Configuration.GetBaseConfiguration(ServiceConfigurationLevel.Profile).Limits.MaxConcurrentPerProfile)
							{
