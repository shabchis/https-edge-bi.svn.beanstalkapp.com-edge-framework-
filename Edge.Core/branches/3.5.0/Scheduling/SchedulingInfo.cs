        ...
        public override string ToString()
        {
            ...
            */
            if (Rule.Scope != SchedulingScope.SingleRun)
				uniqueKey = String.Format("Service:{0}, SelectedDay:{1}, SelectedHour:{2}, RuleScope:{3}, TimeToRun:{4}, ProfileID:{5}", Instance.Configuration.GetBaseConfiguration(ServiceConfigurationLevel.Template).ServiceName, SelectedDay, SelectedHour, Rule.Scope, RequestedTimeStart, Instance.Configuration.GetBaseConfiguration(ServiceConfigurationLevel.Profile).Profile.ID);
            else
            {
